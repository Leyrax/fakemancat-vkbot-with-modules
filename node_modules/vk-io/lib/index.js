...
const DESKTOP_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.80 Safari/537.36';
...
const findUserAgent = headers => {
  ...
  const key = Object.keys(headers).find(header => USER_AGENT_RE.test(header));
...
const CRNL = '\r\n';
class MultipartStream extends sandwichStream.SandwichStream {
...
class WallAttachment extends Attachment {
  ...
  get copyHistory() {
    if (!this[kCopyHistoryAttachments]) {
      this[kCopyHistoryAttachments] = this.payload.copy_history ? this.payload.copy_history.map(history => new WallAttachment(history, this.vk)) : [];
    }
...
class Upload {
  ...
  async video(params) {
    const save = await this.vk.api.video.save(copyParams$1(params, ['group_id', 'album_id', 'link', 'name', 'description', 'is_private', 'wallpost', 'privacy_view', 'privacy_comment', 'no_comments', 'repeat', 'compression']));
  ...
  async document(params) {
    const {
      doc: document
    } = await this.conductDocument(params, {
      attachmentType: 'doc'
    });
    return new DocumentAttachment(document, this.vk);
  }

  async conductWallDocument(params, {
    attachmentType = 'doc'
  } = {}) {
    return this.conduct({
      field: 'file',
  ...
  async wallDocument(params) {
    const {
      doc: document
    } = await this.conductWallDocument(params, {
      attachmentType: 'doc'
    });
    return new DocumentAttachment(document, this.vk);
  }

  async conductMessageDocument(params, {
    attachmentType = 'doc'
  } = {}) {
    return this.conduct({
      field: 'file',
  ...
  async messageDocument(params) {
    const {
      doc: document
    } = await this.conductMessageDocument(params, {
      attachmentType: 'doc'
    });
    return new DocumentAttachment(document, this.vk);
  }
  ...
  async audioMessage(params) {
    ...
    } = await this.conductMessageDocument(_objectSpread({}, params, {
      type: 'audio_message'
    }), {
  ...
  async conduct({
    ...
    if (typeof source !== 'object' || source.constructor !== Object || source.value !== undefined) {
      source = {
        values: source
      };
    }
...
class TypingContext extends Context {
  constructor(vk, payload, {
    ...
    super(vk);
    this.payload = source === updatesSources.POLLING ? transformPolling(payload, updateType) : payload;
    this.type = 'typing';
    this.subTypes = [`typing_${getPeerType(this.fromId)}`];
    this.$groupId = groupId;
  }
...
class MessageReply {
  ...
  [inspect$a.custom](depth, options) {
    ...
    const payload = copyParams(this, ['id', 'conversationMessageId', 'peerId', 'senderId', 'createdAt', 'updatedAt', 'text', 'attachments']);
...
class MessageForward {
  ...
  get forwards() {
    if (!this[kForwards]) {
      this[kForwards] = this.payload.fwd_messages ? this.payload.fwd_messages.map(forward => new MessageForward(forward, this.vk)) : [];
    }
  ...
  [inspect$b.custom](depth, options) {
    ...
    return `${options.stylize(name, 'special')} ${inspect$b(payload, _objectSpread({}, options, {
      compact: false
    }))}`;
  }
...
class MessageContext extends Context {
  constructor(vk, payload, {
    ...
    subTypes.push(!eventType ? subTypesEnum[updateType] : eventType);
  ...
  get eventType() {
    return this.payload.action && this.payload.action.type || null;
  }

  get eventMemberId() {
    return this.payload.action && this.payload.action.member_id || null;
  }

  get eventText() {
    return this.payload.action && this.payload.action.text || null;
  }

  get eventEmail() {
    return this.payload.action && this.payload.action.email || null;
  }
  ...
  get forwards() {
    if (!this[kForwards$1]) {
      this[kForwards$1] = this.payload.fwd_messages ? new MessageForwardsCollection(...this.payload.fwd_messages.map(forward => new MessageForward(forward, this.vk))) : new MessageForwardsCollection();
    }
  ...
  get replyMessage() {
    if (!this[kReplyMessage]) {
      this[kReplyMessage] = this.payload.reply_message ? new MessageReply(this.payload.reply_message, this.vk) : null;
    }
...
class WallPostContext extends Context {
  constructor(vk, payload, {
    ...
    this.type = 'wall_post';
    this.subTypes = [subTypes[updateType]];
  }
...
class UserOnlineContext extends Context {
  constructor(vk, [eventId, userId, extra, date]) {
    ...
    this.subTypes = [subTypes$2[eventId]];
  }
...
class GroupUpdateContext extends Context {
  constructor(vk, payload, {
    ...
    this.subTypes = [subTypes$4[updateType]];
  }
...
class ReadMessagesContext extends Context {
  constructor(vk, [eventId, peerId, id]) {
    ...
    this.subTypes = [subTypes$7[eventId]];
  }
...
class NewAttachmentsContext extends Context {
  constructor(vk, payload, {
    ...
    this.payload = payload;
    this.$groupId = groupId;
    const [subType, Attachment$$1] = subTypes$9[updateType];
...
class RemovedMessagesContext extends Context {
  constructor(vk, [eventId, peerId, id]) {
    ...
    this.subTypes = [subTypes$a[eventId]];
  }
...
class Updates {
  ...
  async startPolling() {
    ...
    try {
      ...
      this.pollingHandler = isGroup ? this.handleWebhookUpdate.bind(this) : this.handlePollingUpdate.bind(this);
  ...
  reloadMiddleware() {
    ...
    if (this.hearStack.length !== 0) {
      stack.push(middlewareIo.getOptionalMiddleware(context => context.type === 'message' && !context.isEvent, middlewareIo.compose([...this.hearStack, this.hearFallbackHandler])));
    }
...
class Keyboard {
  ...
  toString() {
    const buttons = this.buttons.map(buttonRow => buttonRow.map(button => button.toJSON()));
